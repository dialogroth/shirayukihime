...
| ゲームメイン画面 | エンディング画面 | 通常 | 最後の手番フェイズの一周完了（生存プレイヤー全員がプレイ完了した時点で `PHASE_CHANGED { newPhase: "ENDING_QUEEN" }` 受信） |
| ゲームメイン画面 | 勝敗画面 | **強制** | **白雪姫が1分間再接続しなかった場合（`SNOW_WHITE_KILLED { cause: "DISCONNECTED" }` → 3秒後 `GAME_RESULT { reason: "SNOW_WHITE_DISCONNECTED" }`。エンディング画面をスキップ）** |
| ゲームメイン画面 | 勝敗画面 | **強制** | **白雪姫が毒の櫛・呪いの指輪で死亡した場合（`SNOW_WHITE_KILLED { cause: "POISON_COMB" / "CURSED_RING" }` → 3秒後 `GAME_RESULT { reason: "SNOW_WHITE_KILLED" }`。エンディング画面をスキップ）** |
| エンディング画面 | 勝敗画面 | 通常 | エンディングリビール演出完了後、ホストが「結果画面に進む」ボタン押下（`PROCEED_TO_RESULT` 送信 → `GAME_RESULT { reason: "NORMAL" }` 受信） |
...
| ゲームメイン | `WAITING_HOST_PROCEED { nextPhase: "RESULT" }` | ホストには「結果画面に進む」ボタンが表示される。非ホストには「ホストの操作を待っています…」を表示。タイムアウトなし |
| ゲームメイン | `SNOW_WHITE_KILLED { cause }` | 白雪姫死亡の専用演出を3秒間全画面表示（💍 呪いの指輪 / 🪮 毒の櫛 / 📡 接続切断 のいずれか） |
| ゲームメイン | `GAME_STATE_SYNC.turnTimeRemaining` | クライアント側のカウントダウンタイマーと2秒以上ズレている場合のみ補正する。`null` の場合は「3:00」固定表示（エンディング系・終了フェイズ） |
| ゲームメイン | `TURN_CHANGED` | 手番インジケーター更新、カウントダウンタイマー開始（`timeoutSeconds` の値で初期化） |
